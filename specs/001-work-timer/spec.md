...
- 当任务数量为 0 时，系统如何处理？（显示空状态提示和引导按钮，如"暂无任务，点击创建"）
- 当工作记录数量为 0 时，系统如何处理？（显示空状态提示和引导按钮，如"暂无记录，开始记录时间"）
- 当用户同时运行多个应用实例时，系统如何处理？（暂不支持多实例，因为使用无感登录，后续可以扩展多用户并发或并行功能）
...
- **FR-005**：系统必须展示各任务或类别的时间占比（饼图 + 数据表格 + 甘特图），饼图展示周/日/月维度的时间占比，数据表格展示详细数据，甘特图展示单个工作周期内多个工作的执行和切换
...
- **FR-014**：用户必须能够新建任务名称（名称长度限制：英文不超过50个字符，中文不超过20个字符）
- **FR-015**：用户必须能够管理任务信息（编辑、删除等）
- **FR-016**：用户必须能够转移或编辑已有工作记录
- **FR-017**：系统必须在切换任务、开始/结束计时时提供弹窗提示，不同操作显示不同内容（开始/结束/切换显示不同信息）
...
- **任务**：代表一个可记录时间的类别，包含名称、创建时间、是否可删除等属性；默认任务"工作"和"摸鱼"为系统必需；非默认任务名称可以修改
- **工作记录**：代表一段时间的记录，包含开始时间、结束时间、关联任务、持续时间、状态（进行中/已完成/已中断）等属性；记录可编辑、转移
- **时间统计**：按天、周、月维度聚合的工作记录，展示各任务的时间占比和总计；支持饼图、数据表格和甘特图三种展示形式
- **工作周期视图**：用于甘特图展示的单个工作周期视图，展示一个完整工作周期内多个工作的执行和切换；从用户点击"开始"到用户点击"结束"之间的所有工作记录属于一个工作周期
...
- **问**：Windows 端数据库访问方式？ → **答**：使用 SQLModel（而非直接使用 sqlite3），更简洁易懂，提供类型安全和 ORM 能力
- **问**：任务名称是否有最大长度限制？ → **答**：中英文通用，以英文字符为例不超过50个字符，中文则为20个字符
- **问**：是否支持数据导出/导入？ → **答**：支持 JSON 和 CSV 格式的导入和导出
- **问**：非默认任务名称是否可以修改？ → **答**：非默认的工作类型名称可以修改
- **问**：时间统计页面中的"可视化图表或数据汇总"应使用哪种形式？ → **答**：饼图（周、日、月等）+ 数据表格 + 甘特图（单个工作周期，可包含多个工作的执行和切换），需要修改数据结构以支持甘特图
- **问**：工作模式中的"弹窗提示"应包含哪些信息？ → **答**：不同操作显示不同内容（开始/结束/切换显示不同信息）
- **问**：当任务数量为 0 或工作记录数量为 0 时，系统应如何显示？ → **答**：显示空状态提示和引导按钮（如"暂无任务，点击创建"）
- **问**：时间统计查询是否应限制最大时间范围？ → **答**：不限制时间范围（支持查询所有历史记录）
- **问**：当用户同时运行多个应用实例时，系统应如何处理数据一致性？ → **答**：暂不支持多实例，因为使用无感登录，后续可以扩展多用户并发或并行功能